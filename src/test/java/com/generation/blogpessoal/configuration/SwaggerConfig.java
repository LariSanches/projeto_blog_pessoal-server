	...
	OpenAPI springBlogPessoalOpenAPI() {
		return new OpenAPI()
				.info(new Info().title("Projeto Blog Pessoal").description("Projeto Blog Pessoal - Generation Brasil")
						...
						.contact(new Contact().name("Larissa Sanches").url("https://github.com/LariSanches/projeto_blog_pessoal")
								.email("larissaesanches@gmail.com")))
				.externalDocs(new ExternalDocumentation().description("Github")
						.url("https://github.com/LariSanches/projeto_blog_pessoal"));
	}
	...
	OpenApiCustomizer customerGlobalHeaderOpenApiCustomiser() {
		...
		return openApi -> {
			openApi.getPaths().values().forEach(pathItem -> pathItem.readOperations().forEach(operation -> {
				...
				apiResponses.addApiResponse("400", createApiResponse("Erro na Requisiçao!"));
